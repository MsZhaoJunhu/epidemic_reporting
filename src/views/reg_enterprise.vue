			...
			<div class="aui-reg-body">
				<!-- <div class="aui-cell-item">
					...
					<div class="aui-palace aui-palace-one">
						<a href="javascript:;" class="aui-palace-grid" @click="clickToRouter" :class="change == 0?'active':''">
						...
						<a href="javascript:;" class="aui-palace-grid" :class="change == 1?'active':''" v-show="isScanCode">
							<div class="aui-palace-grid-icon">
				...
				<div class="aui-cell-item" v-if="change==0">
					...
					<div class="aui-reg-cell-right">
						<label class="cell-right">
							<router-link to="/login?type=1">已有帐号,去登录</router-link>
						</label>
					</div>
				</div>
				...
				<div class="aui-cell-item" v-else-if="change==1">
					...
					<div class="aui-reg-item-for" v-show="enterpriseType==1">
						<i class="icon icon-phone"></i>
						<input id="phone1" type="text" placeholder="请输入组织机构代码" v-model="enterprise.institutionCode">
					</div>
					<div class="aui-reg-item-for">
						<i class="icon icon-phone"></i>
						<select class="select-style" @change="changeEnterpriseType($event)" :class="enterpriseType!=0?'selected-colorHuiSe':''">
							<option selected value="0" style="display:none;">请选择组织类型</option>
							<option value="1">企业</option>
							<option value="2">事业单位</option>
							<option value="3">医院</option>
							<option value="4">社区</option>
							<!-- <option value="5">园区</option> -->
						</select>
					</div>
					<!-- <div class="aui-reg-item-for" v-show="enterpriseType==1">
						<i class="icon icon-phone"></i>
						<select class="select-style" @change="changepark($event)" :class="parkId!=0?'selected-colorHuiSe':''">
							<option selected value="" style="display:none;">请选择所在园区(无园区可不填)</option>
							<option :value="item.id" v-for="item in parkList">{{item.name}}</option>
						</select>
					</div> -->
					...
					<div class="aui-reg-item-for">
						<i class="icon icon-phone"></i>
						<input id="pasword1" type="password" placeholder="请输入密码" v-model="password">
					</div>
					<div class="aui-reg-item-for">
						<i class="icon icon-phone"></i>
						<input id="pasword2" type="password" placeholder="请再次输入密码" v-model="repassword">
					</div>
					<div class="aui-reg-item-for">
						<i class="icon icon-code"></i>
						<input id="code1" type="text" autocomplete="off" placeholder="请输入验证码" v-model="code">
...
<script>
	export default {
		...
		created() {
			...
			// 	}
			// })
			this.$api.parkList().then(function(res) {
				console.info(res)
				switch (res.data.code) {
					case 200:
						that.parkList = res.data.list
						break;
					case 500:
		...
		methods: {
			...
			changeEnterpriseType(e) {
				this.enterpriseType = e.target.value;
			},
			changepark(e) {
				this.parkId = e.target.value;
			},
			changeArea1(e) {
