	...
	<section class="aui-flexView">
		<header class="aui-navBar aui-navBar-fixed">
			...
			<div class="aui-center">
				<span class="aui-center-title">居民信息记录</span>
			</div>
...
<script>
	export default {
        ...
        methods: {
            checkDetails(status){
                this.$router.push({path:`residentDetailsCompeny/${status}`})
            },
			getPage(page) {
				const that = this
				that.loading = that.$toast.loading();
				this.$api.resident_listCompeny(page).then(function(res) {
                ...
                console.log(that.resumptionList)
			},
        },
	};
</script>
