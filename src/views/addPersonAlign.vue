<template>
	<section class="aui-flexView">
		<header class="aui-navBar aui-navBar-fixed">
			<a href="javascript:;" @click="routerT" class="aui-navBar-item">
				<i class="icon icon-return"></i>
			</a>
			<div class="aui-center">
				<span class="aui-center-title">追加员工</span>
			</div>
			...
			<div class="aui-view-box">
                <div class="person-all-par">
                    <div class="person-par" style="justify-content: space-between;background:none;">
                        ...
                        <button class="resumptionSuccess-btn" @click="confirm">确定</button>
                    </div>
                    <div class="tips-par">
                        提示:红色
                        <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:red;"></span>
                        表示14天出过重庆市,黄色
                        <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:#999;"></span>
                        表示14天出过区域没出重庆市,绿色
                        <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:rgb(35, 223, 98);"></span>
                        表示未出过区域.
                    </div>
...
<script>
	export default {
        ...
        methods: {
            confirm(){
                console.log("点击请求接口在原基础上添加员工")
                const that = this
                let myList=[]
                for (var i = 0; i < that.selectList.length; i++) {
                    myList.push({"id":that.selectList[i].user_id,"status":1});
                }
                that.loading = that.$toast.loading();
                // 请求还未复工的员工接口
                this.$api.appendNumber(myList).then(function(res) {
                    if (res.data.code == 200) {
                        that.loading.hide();
                        that.list=res.data.list
            ...
            routerT(){
                // this.$router.push({path:"resumptionList",query:{type:2}});
				this.$router.back()
            },
